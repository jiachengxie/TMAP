...
import sys
sys.path.append('/home/xie/code/PGP1/models/aggregators')
...
class TMAP(PredictionAggregator):
    """
    ...
    def __init__(self, args: Dict):
        ...
        self.pi_h3 = nn.Linear(args['pi_h2_size'], args['pi_h2_size'])
        self.pi_op_new = nn.Linear(args['pi_h2_size'], 1)   #自己添加
    ...
    def forward(self, encodings: Dict) -> Dict:
        """
        Forward pass for TMAP aggregator
    ...
    def sample_policy(self, pi, s_next, init_node) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for n in range(1, self.horizon): #15个节点长度
                ...
                # gumbel_noise = -torch.log(-torch.log(torch.rand_like(pi_s)))
                # a = F.gumbel_softmax((torch.log(pi_s)+gumbel_noise)/0.1,dim=-1)
                # a = F.gumbel_softmax(pi_s, dim=-1, tau=0.4)
    ...
    def compute_policy(self, target_agent_encoding, node_encodings, node_masks, s_next, edge_type) -> torch.Tensor:
        ...
        # pi_ = self.pi_op_new(self.leaky_relu(self.pi_h3(self.leaky_relu(self.pi_h2(self.leaky_relu(self.pi_h1(enc_batched)))))))
