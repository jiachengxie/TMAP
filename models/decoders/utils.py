...
def cluster_and_rank(k: int, data: np.ndarray):
    ...
    def cluster(n_clusters: int, x: np.ndarray):
        ...
        # clustering_op = EDKMeans(n_clusters=n_clusters, data=x, a=0.5, r=2)

        # 一.自己增加的,用卡尔曼增益修正
        ukf = load_ukf_params('/home/xie/code/TMAP/UKF_params')
        centers = np.array([])
        len_traj = int(x.data.shape[1] / 2 * 3)
        ukf_pre = predict_next_state(ukf, len_traj)  # (len_traj/3,5)
        ukf_p = ukf_pre[0::3, :]  # (len_traj/3,5)
        ukf_p = ukf_p.reshape(1, -1)  # (1,len_traj/3*2)
        for cen in clustering_op.cluster_centers_: # clustering_op.cluster_centers_:(10,1,len_traj/3*2),cen:(1,len_traj/3*2)
            for index in range(0,len(cen),2):
                x1 = copy.deepcopy(ukf_p[0][int(index * 5 / 2):int(index * 5 / 2) + 5])
                z1 = copy.deepcopy(cen[index:index + 2])
                center = correct(ukf,x1,z1)
                centers = np.append(centers, center)
        centers = centers.reshape(n_clusters, -1)
        return clustering_op.labels_,centers
