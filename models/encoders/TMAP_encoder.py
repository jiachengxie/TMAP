...
import sys
print(sys.path)
sys.path.append('/home/xie/code/TMAP/models/encoders')
import target_surround
import u_gru
...
class TMAPEncoder(PredictionEncoder):

    def __init__(self, args: Dict):
        """
        GRU based encoder from TMAP. Lane node features and agent histories encoded using GRUs.
        ...
        self.target_agent_enc = nn.GRU(args['target_agent_emb_size'], args['target_agent_enc_size'], batch_first=True,
                                       num_layers=1)
        self.target_agent_enc_u = u_gru.GRU(args['target_agent_emb_size'], args['target_agent_enc_size'])  #自己添加ugru

        # Node encoders
        self.node_emb = nn.Linear(args['node_feat_size'], args['node_emb_size'])
        self.node_encoder = nn.GRU(args['node_emb_size'], args['node_enc_size'], batch_first=True, num_layers=1)
        self.node_encoder_u = u_gru.GRU(args['node_emb_size'], args['node_enc_size'])  #addition

        # Surrounding agent encoder
        self.nbr_emb = nn.Linear(args['nbr_feat_size'] + 1, args['nbr_emb_size'])
        self.nbr_enc = nn.GRU(args['nbr_emb_size'], args['nbr_enc_size'], batch_first=True, num_layers=1)
        self.nbr_enc_u = u_gru.GRU(args['nbr_emb_size'], args['nbr_enc_size'])  #自己添加
        ...
        self.target_surround = target_surround.Traget_surround(args['target_agent_enc_size'], args['nbr_enc_size'])
        ...
        self.gat1 = nn.ModuleList([GAT(args['node_enc_size'], args['node_enc_size'])
                                  for _ in range(args['num_gat_layers'])])

    def forward(self, inputs: Dict) -> Dict:
        """
        Forward pass for TMAP encoder
        ...
        # target_agent_enc = target_agent_enc[0:1, :, :]
        # target_agent_enc = self.target_agent_enc_u(target_agent_embedding)
        target_agent_enc1 = target_agent_enc.squeeze(0)   #zijitianjiade
        ...
        # target_surround attention
        nbr_encodings = torch.cat((nbr_vehicle_enc, nbr_ped_enc), dim=1)  # 周围编码将行人和车辆编码按照对二维拼接
        queries = self.query_emb(nbr_encodings).permute(1, 0, 2)  # 道路节点查询生成[161, 64, 32]
        keys_tra = self.key_emb(target_agent_enc)  # 周围编码k的生成[1, 64, 32]
        vals_tra = self.val_emb(target_agent_enc)  # 周围编码v的生成[1, 64, 32]
        target_agent_masks = torch.cat((inputs['agent_node_masks']['vehicles'],
                                        inputs['agent_node_masks']['pedestrians']), dim=2)
        target_agent_masks = target_agent_masks[:, 0:1, :]
        target_agent_masks = target_agent_masks.permute(0, 2, 1)
        target_agent_enc, _ = self.a_n_att(queries, keys_tra, vals_tra,
                                           attn_mask=target_agent_masks)  # 目标节点多头注意力[161, 64, 32]
        target_agent_enc = target_agent_enc.permute(1, 0, 2)  # 自己添加
        target_agent_enc = target_agent_enc.reshape(target_agent_enc.size(0), -1)
        mix1 = nn.Linear(target_agent_enc.shape[1], target_agent_enc.shape[1] // 8).to(device)
        mix2 = nn.Linear(target_agent_enc.shape[1] // 8, 32).to(device)
        target_agent_enc = mix2(F.leaky_relu(mix1(target_agent_enc))).to(device)
        target_agent_enc = target_agent_enc + target_agent_enc1
        target_agent_enc = target_agent_enc.unsqueeze(1)
        ...
        adj_mat = adj_mat[:, 0:1, 0:1]
        for gat_layer in self.gat:
            target_agent_enc = target_agent_enc + gat_layer(target_agent_enc, adj_mat)
        target_agent_enc = target_agent_enc[:, 0, :] + target_agent_enc1   #自己添加GAT

        # target_agent_enc1 = self.target_surround(target_agent_enc, nbr_encodings) #自己添加
        # target_agent_enc = self.leaky_relu(self.mix(torch.cat((target_agent_enc, target_agent_enc1), dim=1)))
    ...
    def variable_size_gru_encode(feat_embedding: torch.Tensor, masks: torch.Tensor, gru: nn.GRU) -> torch.Tensor:
        ...
        if len(seq_lens_batched) != 0:
            ...
            # feat_embedding_packed, lengths = pad_packed_sequence(feat_embedding_packed, batch_first=True)
            # Encode
            _, encoding_batched = gru(feat_embedding_packed)
            # encoding_batched = gru(feat_embedding_packed)
            ...
            hidden_state_size = gru.hidden_size   #original
